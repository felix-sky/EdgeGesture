...
Item {
    ...
    height: loader.status === Loader.Ready && loader.item ? loader.item.implicitHeight : 32
    ...
    property var editor: null // Add editor property
    ...
    Component {
        ...
        Text {
            ...
            MouseArea {
                ...
                onClicked: mouse => {
                    console.log("HeadingBlock: Clicked, noteListView:", root.noteListView ? "available" : "null");
                    console.log("HeadingBlock: blockIndex:", root.blockIndex);
                    // Headings don't usually have links, but to be consistent:
                    if (root.noteListView) {
                        root.noteListView.currentIndex = root.blockIndex;
                        root.isEditing = true;
                        console.log("HeadingBlock: Set isEditing to true");
                    } else {
                        console.log("HeadingBlock: noteListView is null, cannot edit");
                    }
                }
            }
        }
